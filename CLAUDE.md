...
- 下一步：编译核对与构建落地（补装 JDK 17 + Maven；对照 kwik 0.9.x javadoc 校正 `QuicTransportEndpoint`/`QuicTransportChannel` 中标注 `待对照 javadoc` 的 API 签名）；Phase 2 富媒体深化（H.264 硬编替换 JPEG 占位、`DesktopDuplicationCapturer` DXGI 原生抓屏、回切后的通道预热/首帧优化）
